...
public class SearchBleAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        String dName = bluetoothDevice.getName() == null ? com.shop.shopping.utils.TextUtils.getString(R.string.unkonw_device) : bluetoothDevice.getName();
        if (TextUtils.isEmpty(dName)) {
            dName = com.shop.shopping.utils.TextUtils.getString(R.string.unkonw_device);
        }
        holder.name.setText(dName);
        String dAddress = bluetoothDevice.getAddress() == null ? com.shop.shopping.utils.TextUtils.getString(R.string.unkonw_address) : bluetoothDevice.getAddress();
        if (TextUtils.isEmpty(dAddress)) {
            dAddress = com.shop.shopping.utils.TextUtils.getString(R.string.unkonw_address);
        }
        ...
        if (bluetoothDevice.getBondState() == BluetoothDevice.BOND_BONDED) {
            if (dAddress.equals(mConnectedDeviceAddress)) {
                holder.bond.setText(R.string.connected);
                holder.bond.setPadding(paddingHorizontal, paddingVertical, paddingHorizontal, paddingVertical);
            } else {
                holder.bond.setText(R.string.paired);
                holder.bond.setPadding(paddingHorizontal, paddingVertical, paddingHorizontal, paddingVertical);
            }
        } else {
            holder.bond.setText(R.string.un_pair);
            holder.bond.setPadding(paddingHorizontal, paddingVertical, paddingHorizontal, paddingVertical);
        }
        return convertView;
    }
